...
describe('ZekoBridge', () => {
  ...
  it('dispatches three wrapping requests', async () => {
    ...
    const tx = await Mina.transaction(senderAddress, () => {
      zkapp.dispatchMinaWrappingRequest(pendingActions[0]);
      zkapp.dispatchMinaWrappingRequest(pendingActions[1]);
      zkapp.dispatchMinaWrappingRequest(pendingActions[2]);
    });
  ...
  it('rolls up actions using proof', async () => {
    ...
    expect(zkapp.treeRoot.get()).toEqual(proof.publicInput.treeRoot);
    expect(zkapp.counter.get()).toEqual(proof.publicInput.counter);
    expect(zkapp.actionState.get()).toEqual(proof.publicInput.actionState);
  ...
  it('rolls up actions using proof and wraps the queue around the tree', async () => {
    ...
    console.log(zkapp.lastIndex().toBigInt());
