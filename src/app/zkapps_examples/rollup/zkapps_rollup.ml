...
struct
  ...
  module Mocked_zkapp = struct
    module Deploy = struct
      let deploy ~(signer : Signature_lib.Keypair.t) ~(fee : Currency.Fee.t)
        ...
        { command with
          fee_payer =
            { command.fee_payer with authorization = sender_signature }
        ; account_updates =
            Zkapp_command.Call_forest.accumulate_hashes
              ~hash_account_update:(fun p ->
                Zkapp_command.Digest.Account_update.create p )
    ...
    module Step = struct
      ...
      let main { public_input = () } =
        ...
        in
        let public_output, auxiliary_output =
          make_outputs account_update calls
        in
        { previous_proof_statements =
  ...
  module Mocked = struct
    ...
    let step pk vk_hash (snark : Wrapper_rules.t) =
      step_
        ~handler:
          (Mocked_zkapp.Step.handler pk vk_hash snark.statement snark.proof)
