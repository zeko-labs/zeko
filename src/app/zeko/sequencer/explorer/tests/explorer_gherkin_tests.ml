(* Executes minimal Gherkin acceptance tests for the explorer-facing behavior
   introduced in this PR: event payloads and backfill GraphQL/SSE behavior. *)
...
let graphql_data_exn service query field =
  find_assoc_exn field (find_assoc_exn "data" (graphql_response service query))

let pipe_read_exn reader =
  Thread_safe.block_on_async_exn (fun () ->
      Pipe.read reader
      >>| function `Ok value -> value | `Eof -> failwith "Unexpected EOF" )
...
let add_job (service : Explorer_backfill_service.t)
    ?(status = Explorer_backfill_service.Queued) ?(diffs_published = 0) ?error
    ?started_at ?finished_at ?(id = "job-1") () =
...
let sse_reader_exn service body =
  ...
  Thread_safe.block_on_async_exn (fun () ->
      match%bind
        Explorer_backfill_sse.execute_subscription service req body
      with
      | Error err ->
...
let parse_sse_next_event_exn event =
  ...
  if not (String.is_prefix event ~prefix) then
    failwithf "Unexpected SSE event: %s" event ()
  else
...
let assert_basic_json_equal actual expected =
  if not (Yojson.Basic.equal actual expected) then
    failwithf "Expected %s but got %s"
      (Yojson.Basic.to_string expected)
      (Yojson.Basic.to_string actual)
      ()

let assert_safe_json_equal actual expected =
  if not (Yojson.Safe.equal actual expected) then
    failwithf "Expected %s but got %s"
...
let%test_unit "Backfill progress subscriptions stream job updates" =
  ...
  in
  let initial = pipe_read_string_exn reader |> backfill_progress_event_exn in
  ...
  Explorer_backfill_service.update_job job ~status:Running ~diffs_published:1
    ~started_at:Time.epoch () ;
  let first = pipe_read_string_exn reader |> backfill_progress_event_exn in
  assert_basic_json_equal (find_assoc_exn "diffsPublished" first) (`Int 1) ;
  Explorer_backfill_service.update_job job ~status:Completed ~diffs_published:2
    ~finished_at:Time.epoch () ;
  let final_progress =
    pipe_read_string_exn reader |> backfill_progress_event_exn
  in
  assert_basic_json_equal
    (find_assoc_exn "diffsPublished" final_progress)
    (`Int 2) ;
  assert_basic_json_equal
    (find_assoc_exn "status" final_progress)
    (`String "completed")
...
let%test_unit "The backfill health query exposes the service instance" =
  ...
  let health =
    graphql_data_exn (test_service ())
      {|query { health { ok instanceId startedAt } }|} "health"
  in
...
let%test_unit "The backfill job query returns the current job snapshot" =
  ...
  let backfill_job =
    graphql_data_exn service
      (sprintf {|query { backfillJob(id: "%s") { id status diffsPublished } }|}
         job.id )
      "backfillJob"
  in
...
let%test_unit "Backfill progress subscriptions stream GraphQL-SSE events" =
  ...
  Explorer_backfill_service.update_job job ~status:Running ~diffs_published:1
    ~started_at:Time.epoch () ;
  let running = pipe_read_string_exn reader |> backfill_progress_event_exn in
  assert_basic_json_equal (find_assoc_exn "diffsPublished" running) (`Int 1) ;
  Explorer_backfill_service.update_job job ~status:Completed ~diffs_published:2
    ~finished_at:Time.epoch () ;
  let completed = pipe_read_string_exn reader |> backfill_progress_event_exn in
  assert_basic_json_equal
    (find_assoc_exn "status" completed)
    (`String "completed") ;
  [%test_eq: string]
...
let%test_unit
  ...
  let message =
    Explorer_events.build_finality_message ~logger:(Logger.create ())
      ...
      ~source_ledger_hash:Ledger_hash.empty_hash ~target_ledger_hash
  in
...
let%test_unit "Health events include the service identity and publishing state"
  ...
  let message =
    Explorer_events.build_health_message ~logger:(Logger.create ())
      ~service:"sequencer-nats-publisher" ~instance_id:"instance-1" ~status:"ok"
      ~last_published_hash:Ledger_hash.empty_hash
  ...
  assert_safe_json_equal
    (find_assoc_exn "instance_id" message.payload)
    (`String "instance-1") ;
  assert_safe_json_equal (find_assoc_exn "status" message.payload) (`String "ok") ;
  assert_safe_json_equal
    (find_assoc_exn "last_published_hash" message.payload)
    (Ledger_hash.to_yojson Ledger_hash.empty_hash) ;
  assert_safe_json_equal
    (find_assoc_exn "unproved_hash" message.payload)
    (Ledger_hash.to_yojson Ledger_hash.empty_hash)
...
let%test_unit "Backfill dropped publishes are fatal" =
  Feature_parser.assert_scenario "event-contract.feature"
    "Backfill dropped publishes fail the job" ;
  let service = test_service () in
  let job =
  ...
  in
  match
    Explorer_backfill_service.handle_backfill_publish_result service job
      ~target_ledger_hash:Ledger_hash.empty_hash `Dropped
  with
  | Ok _ ->
      failwith "expected dropped backfill publish to be fatal"
  | Error error ->
      if
        not
          (String.is_substring (Error.to_string_hum error)
             ~substring:"NATS publish dropped" )
      then
        failwithf "unexpected dropped-publish error: %s"
          (Error.to_string_hum error)
          () ;
      [%test_eq: int] job.diffs_published 0
