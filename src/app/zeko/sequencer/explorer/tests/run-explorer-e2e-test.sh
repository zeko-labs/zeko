...
usage() {
  echo "Usage: $0 <fake|real> <num_provers> [wait_for_port?]" >&2
  exit 1
}
...
if [ -z "${NATS_URL:-}" ]; then
  echo "Error: NATS_URL must point at a running NATS server" >&2
  exit 1
fi
...
SEQUENCER_BUILD_ROOT="$REPO_ROOT/_build/default/src/app/zeko/sequencer"
SIGNER_BUILD_ROOT="$REPO_ROOT/_build/default/src/app/zeko/signer"
...
if [ "$MODE" = "fake" ]; then
  sleep 5
  PROVER_BIN="$SEQUENCER_BUILD_ROOT/prover/cli_fake.exe"
else
...
for ((i = 0; i < NUM_PROVERS; i++)); do
  run "prover-$i" \
    ...
    "$PROVER_BIN" run-server --mq-host "localhost:5672" &
  PROVER_PIDS+=("$!")
done
