...
module Queue = struct
  type 'a node = { data : 'a; mutable next : 'a node option } [@@deriving sexp]
  ...
  let dequeue_exn t =
    match t.head with
    | None ->
        raise_s [%message "Queue.dequeue_exn: empty queue"]
    | Some node ->
        let data = node.data in
        t.head <- node.next ;
        node.next <- None ;
        t.length <- t.length - 1 ;
        if t.length = 0 then t.tail <- None ;
        data

  let push_front t a =
    let node = { data = a; next = t.head } in
    t.length <- t.length + 1 ;
    match t.head with
    | None ->
        t.head <- Some node ;
...
let invariant invariant_a t : unit =
  try
    ...
    Fields.iter ~continue_on_error:ignore
      ~max_concurrent_jobs:
      ...
      ~num_resources_not_cleaned:
        (check (fun num_resources_not_cleaned ->
             ...
             if num_resources_not_cleaned < t.max_concurrent_jobs then
               assert t.is_dead ) )
      ~cleaned:
        (check (fun cleaned ->
             if Ivar.is_full cleaned then
               assert (t.num_resources_not_cleaned = 0) ) )
  with exn ->
    raise_s [%message "Throttle.invariant failed" (exn : exn) (t : _ t)]

module T2 = struct
  type nonrec ('a, 'kind) t = 'a t
...
let at_kill t f =
  ...
  let f =
    unstage (Monitor.Exported_for_scheduler.preserve_execution_context' f)
  in
  t.cleans <- f :: t.cleans
...
let rec start_job t =
  ...
  | `Ok ->
      ()
  | `Raised ->
      if not t.continue_on_error then kill t ) ;
  if t.is_dead then clean_resource t job_resource
  else (
    Stack.push t.job_resources_not_in_use job_resource ;
    if not (Queue.is_empty t.jobs_waiting_to_start) then start_job t
    else
...
let create_internal ~continue_on_error job_resources =
  let max_concurrent_jobs = Stack.length job_resources in
  { continue_on_error
  ; max_concurrent_jobs
...
let create_with ~continue_on_error job_resources =
  create_internal ~continue_on_error (Stack.of_list job_resources)
...
let enqueue' t f =
  ...
  if t.is_dead then Job.abort job
  else (
...
let push_front' t f =
  ...
  if t.is_dead then Job.abort job
    ...
    Queue.push_front t.jobs_waiting_to_start job.internal_job ;
    if t.num_jobs_running < t.max_concurrent_jobs then start_job t ) ;
  Job.result job
...
let handle_enqueue_result result =
  ...
  | `Ok a ->
      a
  | `Aborted ->
      raise_s [%message "throttle aborted job"]
  | `Raised exn ->
      raise exn

let enqueue t f = enqueue' t f >>| handle_enqueue_result

let push_front t f = push_front' t f >>| handle_enqueue_result
