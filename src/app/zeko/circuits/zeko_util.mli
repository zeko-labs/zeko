...
val value_to_fields :
  ('var, 'value) Snark_params.Tick.Typ.t -> 'value -> Pasta_bindings.Fp.t array

val var_to_fields :
  ('var, 'value) Snark_params.Tick.Typ.t -> 'var -> Field.Var.t array

val var_to_field : ('var, 'value) Snark_params.Tick.Typ.t -> 'var -> Field.Var.t

val unsaf_var_of_fields :
  ('var, 'value) Snark_params.Tick.Typ.t -> Field.Var.t array -> 'var
...
val unsafe_var_of_app_state :
  ...
  -> Field.Var.t Pickles_types.Vector.Vector_8.t
  -> 'var

val proof_permissions : Mina_base.Permissions.t
...
module Calls : sig
  type t = [] | ( :: ) of (Mina_base.Account_update.Checked.t * t) * t
end
...
module Var_to_precondition_fine : sig
  type t =
    | [] : t
    | ( :: ) : (('var, 't) Snark_params.Tick.Typ.t * 'var option) * t -> t
end
...
val value_to_init_state :
     ('a, 'b) Snark_params.Tick.Typ.t
  -> 'b
  -> Pasta_bindings.Fp.t Pickles_types.Vector.Vector_8.t
...
val var_to_actions :
     ('var, 'value) Snark_params.Tick.Typ.t
  -> 'var
  -> Mina_base.Zkapp_account.Actions.var Checked.t

val value_to_actions :
     ('var, 'value) Snark_params.Tick.Typ.t
  -> 'value
  -> Mina_base.Zkapp_account.Actions.t
...
module MkHandler : functor (Witness : SnarkType) -> sig
  ...
  val exists_witness : Witness.var Snark_params.Tick.Checked.t
end
...
val assert_var :
     string
  -> (unit -> Snark_params.Tick.Boolean.var)
  -> unit Snark_params.Tick.Checked.t

val default_account_update : Mina_base.Account_update.Checked.t

val ( let* ) :
     'a Snark_params.Tick.Checked.t
  -> ('a -> 'b Snark_params.Tick.Checked.t)
  -> 'b Snark_params.Tick.Checked.t

val ( let*| ) :
  'a Snark_params.Tick.Checked.t -> ('a -> 'b) -> 'b Snark_params.Tick.Checked.t
