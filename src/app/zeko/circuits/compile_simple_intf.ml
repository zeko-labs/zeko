...
module Compile_simple (I : Inputs) = struct
  open I
  ...
  type ('var, 'max_proofs_verified) prev =
    { public_input : 'var
    ; proof : ('max_proofs_verified, 'max_proofs_verified) Pickles.Proof.t v_t
  ...
  type ('input, 'out_var, 'prevs) branch =
    ...
    ; main : 'input v_t -> ('out_var, 'prevs) main_return Checked.t
    }
