...
module Make (Inputs : Inputs_intf) : sig
  ...
  let create ~depth () = { uuid = Uuid.create_random (Random.State.make_self_init ()); depth }
  ...
  let accounts _t = Deferred.return Account_id.Set.empty
  ...
  let fold_until _t ~init ~f:_ ~finish = Deferred.return @@ finish init

  let foldi_with_ignored_accounts _t _ ~init ~f:_ = init

  let foldi _t ~init ~f:_ = init

  let to_list _t = Deferred.return []
